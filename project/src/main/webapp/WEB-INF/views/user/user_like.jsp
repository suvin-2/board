			...
			<div class="tab_container">
				...
				<div class="tabs">
					<ul>
						...
						<li class="active"><a href="#" rel="tab3">좋아요 글</a></li>
					</ul>
				</div>
				<!-- 좋아요 한 글 출력 -->
				<div id="tab3" class="tab_content">
					<table border="1" id="user_like_table">
						...
						<tbody id="like_tbody">
							<c:forEach var="item" items="${like_list}">
							...
							<fmt:formatDate var="bDate" value="${item.bDate}" pattern="yyyy-MM-dd HH:MM"/>
								<tr onClick = "${url}">
								    <td id="like_sName">${item.sName}</td>
								    <td id="like_title">${item.title}</td>
								    <td id="like_writer">${item.writer}</td>
								    <td id="like_bDate">${bDate}</td>
								    <td id="like_cnt">${item.cnt}</td>
								</tr>
						    </c:forEach>
						</tbody>
					</table>	
					...
					<div class="pagingBnt">
						  <ul class="pagination" id="paging_ul">
						    <c:if test="${like_pageMaker.prev}"> 
						    <li> 
						        <a href='<c:url value="userWritingList.do?userId=${userId}&page=${like_pageMaker.startPage-1}"/>' class="button"></a>
						    </li>
						    </c:if>
						    <c:forEach begin="${like_pageMaker.startPage}" end="${like_pageMaker.endPage}" var="pageNum">
						    <li>
						        <a href='<c:url value="userWritingList.do?userId=${userId}&page=${pageNum}"/>' class="page">${pageNum}</a>
						    </li>
						    </c:forEach>
						    <c:if test="${like_pageMaker.next && like_pageMaker.endPage>0}">
						    <li>
						        <a href='<c:url value="userWritingList.do?userId=${userId}&page=${like_pageMaker.endPage+1}"/>' class="button"></a>
						    </li>
						    </c:if>
						</ul>
					</div>
